...
namespace OSDevGrp.OSIntranet.BusinessLogic.Tests.MediaLibrary.Commands.CreateLendingCommand
{
	[TestFixture]
	public class ValidateTests
	{
		...
		{
			ICreateLendingCommand sut = CreateSut();
		...
		[Test]
		[Category("UnitTest")]
		public void Validate_WhenCalled_AssertShouldBeKnownValueWasNotCalledOnObjectValidatorWithLendingIdentifier()
		{
			Guid lendingIdentifier = Guid.NewGuid();
			ICreateLendingCommand sut = CreateSut(lendingIdentifier);

			sut.Validate(_validatorMockContext.ValidatorMock.Object, _claimResolverMock.Object, _mediaLibraryRepositoryMock.Object, _commonRepositoryMock.Object);

			_validatorMockContext.ObjectValidatorMock.Verify(m => m.ShouldBeKnownValue(
					It.Is<Guid>(value => value == lendingIdentifier),
		...
		[Test]
		[Category("UnitTest")]
		public void Validate_WhenCalled_AssertShouldBeUnknownValueWasCalledOnObjectValidatorWithLendingIdentifier()
		{
			Guid lendingIdentifier = Guid.NewGuid();
			ICreateLendingCommand sut = CreateSut(lendingIdentifier);
			...
			_validatorMockContext.ObjectValidatorMock.Verify(m => m.ShouldBeUnknownValue(
					...
					It.Is<string>(value => string.IsNullOrWhiteSpace(value) == false && string.CompareOrdinal(value, "LendingIdentifier") == 0),
		...
		private ICreateLendingCommand CreateSut(Guid? lendingIdentifier = null)
		{
			_claimResolverMock.Setup(m => m.IsMediaLibraryLender())
				.Returns(_fixture.Create<bool>());

			return MediaLibraryCommandFactory.BuildCreateLendingCommand(lendingIdentifier ?? Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid(), DateTime.Today.AddDays(_random.Next(21, 365) * -1), DateTime.Today.AddDays(_random.Next(7, 14) * -1), _random.Next(100 ) > 50 ? DateTime.Today.AddDays(_random.Next(7, 14) * -1) : null);
		}
	}
}