...
namespace OSDevGrp.OSIntranet.BusinessLogic.Tests.MediaLibrary.Commands.DeleteBorrowerCommand
{
	[TestFixture]
	public class ValidateTests
	{
		...
		{
			IDeleteBorrowerCommand sut = CreateSut();
		...
		public void Validate_WhenCalled_AssertShouldBeKnownValueWasCalledOnObjectValidatorWithBorrowerIdentifier()
		{
			...
			IDeleteBorrowerCommand sut = CreateSut(borrowerIdentifier);
		...
		[Test]
		[Category("UnitTest")]
		public void Validate_WhenCalled_AssertShouldBeDeletableWasCalledOnObjectValidatorWithBorrowerIdentifier()
		{
			Guid borrowerIdentifier = Guid.NewGuid();
			IDeleteBorrowerCommand sut = CreateSut(borrowerIdentifier);
			...
			_validatorMockContext.ObjectValidatorMock.Verify(m => m.ShouldBeDeletable(
					It.Is<Guid>(value => value == borrowerIdentifier),
					It.IsNotNull<Func<Guid, Task<IBorrower>>>(),
					It.Is<Type>(value => value != null && value == sut.GetType()),
					It.Is<string>(value => string.IsNullOrWhiteSpace(value) == false && string.CompareOrdinal(value, "BorrowerIdentifier") == 0),
					It.Is<bool>(value => value == false)),
				Times.Once);
		...
		private IDeleteBorrowerCommand CreateSut(Guid? borrowerIdentifier = null)
		{
			_claimResolverMock.Setup(m => m.IsMediaLibraryModifier())
				.Returns(_fixture.Create<bool>());

			return MediaLibraryCommandFactory.BuildDeleteBorrowerCommand(borrowerIdentifier ?? Guid.NewGuid());
		}
	}
}