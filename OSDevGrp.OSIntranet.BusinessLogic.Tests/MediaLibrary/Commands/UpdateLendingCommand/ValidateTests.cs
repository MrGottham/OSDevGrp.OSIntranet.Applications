...
namespace OSDevGrp.OSIntranet.BusinessLogic.Tests.MediaLibrary.Commands.UpdateLendingCommand
{
	[TestFixture]
	public class ValidateTests
	{
		...
		{
			IUpdateLendingCommand sut = CreateSut();
		...
		public void Validate_WhenCalled_AssertShouldBeKnownValueWasCalledOnObjectValidatorWithLendingIdentifier()
		{
			...
			IUpdateLendingCommand sut = CreateSut(lendingIdentifier);
		...
		private IUpdateLendingCommand CreateSut(Guid? lendingIdentifier = null)
		{
			_claimResolverMock.Setup(m => m.IsMediaLibraryLender())
				.Returns(_fixture.Create<bool>());

			return MediaLibraryCommandFactory.BuildUpdateLendingCommand(lendingIdentifier ?? Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid(), DateTime.Today.AddDays(_random.Next(21, 365) * -1), DateTime.Today.AddDays(_random.Next(7, 14) * -1), _random.Next(100 ) > 50 ? DateTime.Today.AddDays(_random.Next(7, 14) * -1) : null);
		}
	}
}