...
namespace OSDevGrp.OSIntranet.BusinessLogic.Tests.MediaLibrary.Commands.UpdateBookCommand
{
    [TestFixture]
	public class ValidateTests
	{
		...
		{
			IUpdateBookCommand sut = CreateSut();
		...
		public void Validate_WhenCalled_AssertShouldBeKnownValueWasCalledOnObjectValidatorWithMediaIdentifier()
		{
			...
			IUpdateBookCommand sut = CreateSut(mediaIdentifier);
		...
		private IUpdateBookCommand CreateSut(Guid? mediaIdentifier = null)
		{
			_claimResolverMock.Setup(m => m.IsMediaLibraryModifier())
				.Returns(_fixture.Create<bool>());

			return MediaLibraryCommandFactory.BuildUpdateBookCommand(mediaIdentifier ?? Guid.NewGuid(), _fixture.Create<string>().ToUpper(), _random.Next(100) > 50 ? _fixture.Create<string>().ToUpper() : null, _random.Next(100) > 50 ? _fixture.Create<string>() : null, _random.Next(100) > 50 ? _fixture.Create<string>() : null, _fixture.Create<int>(), _random.Next(100) > 50 ? _fixture.Create<int>() : null, _fixture.Create<int>(), _random.Next(100) > 50 ? _fixture.Create<string>() : null, _random.Next(100) > 50 ? _fixture.Create<short>() : null, _random.Next(100) > 50 ? _fixture.CreateEndpointString(path: $"api/book/{_fixture.Create<string>()}") : null, _random.Next(100) > 50 ? _fixture.CreateMany<byte>(_random.Next(1024, 4096)).ToArray() : Array.Empty<byte>(), _fixture.CreateMany<Guid>(_random.Next(1, 7)).ToArray());
		}
	}
}