...
namespace OSDevGrp.OSIntranet.BusinessLogic.Tests.MediaLibrary.Commands.BorrowerIdentificationCommandBase
{
	[TestFixture]
	public class ValidateTests
	{
		...
		{
			IBorrowerIdentificationCommand sut = CreateSut();
		...
		private IBorrowerIdentificationCommand CreateSut()
		{
			_claimResolverMock.Setup(m => m.IsMediaLibraryLender())
				.Returns(_fixture.Create<bool>());

			return new MyBorrowerIdentificationCommand(Guid.NewGuid());
		}

		private class MyBorrowerIdentificationCommand : BusinessLogic.MediaLibrary.Commands.BorrowerIdentificationCommandBase
		{
			#region Constructor

			public MyBorrowerIdentificationCommand(Guid borrowerIdentifier) 
