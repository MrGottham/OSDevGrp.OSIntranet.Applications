...
namespace OSDevGrp.OSIntranet.BusinessLogic.Tests.MediaLibrary.Commands.UpdateMediaPersonalityCommand
{
    [TestFixture]
	public class ValidateTests
	{
		...
		{
			IUpdateMediaPersonalityCommand sut = CreateSut();
		...
		[Test]
		[Category("UnitTest")]
		public void Validate_WhenCalled_AssertShouldBeKnownValueWasCalledOnObjectValidatorWithMediaPersonalityIdentifier()
		{
			Guid mediaPersonalityIdentifier = Guid.NewGuid();
			IUpdateMediaPersonalityCommand sut = CreateSut(mediaPersonalityIdentifier);
		...
		[Test]
		[Category("UnitTest")]
		public void Validate_WhenCalled_AssertShouldBeUnknownValueWasNotCalledOnObjectValidatorWithMediaPersonalityIdentifier()
		{
			Guid mediaPersonalityIdentifier = Guid.NewGuid();
			IUpdateMediaPersonalityCommand sut = CreateSut(mediaPersonalityIdentifier);
		...
		private IUpdateMediaPersonalityCommand CreateSut(Guid? mediaPersonalityIdentifier = null)
		{
			_claimResolverMock.Setup(m => m.IsMediaLibraryModifier())
				.Returns(_fixture.Create<bool>());

			return MediaLibraryCommandFactory.BuildUpdateMediaPersonalityCommand(mediaPersonalityIdentifier ?? Guid.NewGuid(), _random.Next(100) > 50 ? _fixture.Create<string>() : null, _random.Next(100) > 50 ? _fixture.Create<string>() : null, _fixture.Create<string>(), _fixture.Create<int>(), _random.Next(100) > 50 ? DateTime.Today.AddDays(_random.Next(1, 365) * -1) : null, _random.Next(100) > 50 ? DateTime.Today.AddDays(_random.Next(1, 365) * -1) : null, _random.Next(100) > 50 ? _fixture.CreateEndpointString(path: $"api/mediaPersonality/{_fixture.Create<string>()}") : null, _random.Next(100) > 50 ? _fixture.CreateMany<byte>(_random.Next(1024, 4096)).ToArray() : Array.Empty<byte>());
		}
	}
}