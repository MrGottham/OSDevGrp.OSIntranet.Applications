...
namespace OSDevGrp.OSIntranet.BusinessLogic.Tests.MediaLibrary.Commands.UpdateBorrowerCommand
{
	[TestFixture]
	public class ValidateTests
	{
		...
		{
			IUpdateBorrowerCommand sut = CreateSut();
		...
		public void Validate_WhenCalled_AssertShouldBeKnownValueWasCalledOnObjectValidatorWithBorrowerIdentifier()
		{
			...
			IUpdateBorrowerCommand sut = CreateSut(borrowerIdentifier);
		...
		[Test]
		[Category("UnitTest")]
		public void Validate_WhenCalled_AssertShouldBeUnknownValueWasNotCalledOnObjectValidatorWithBorrowerIdentifier()
		{
			Guid borrowerIdentifier = Guid.NewGuid();
			IUpdateBorrowerCommand sut = CreateSut(borrowerIdentifier);
		...
		private IUpdateBorrowerCommand CreateSut(Guid? borrowerIdentifier = null)
		{
			_claimResolverMock.Setup(m => m.IsMediaLibraryLender())
				.Returns(_fixture.Create<bool>());

			return MediaLibraryCommandFactory.BuildUpdateBorrowerCommand(borrowerIdentifier ?? Guid.NewGuid(), _fixture.Create<string>(), _random.Next(100) > 50 ? _fixture.Create<string>() : null, _random.Next(100) > 50 ? _fixture.Create<string>() : null, _random.Next(100) > 50 ? _fixture.Create<string>() : null, _fixture.Create<int>());
		}
	}
}