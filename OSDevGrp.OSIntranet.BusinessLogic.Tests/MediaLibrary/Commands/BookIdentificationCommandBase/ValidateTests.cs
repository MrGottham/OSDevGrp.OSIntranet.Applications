...
namespace OSDevGrp.OSIntranet.BusinessLogic.Tests.MediaLibrary.Commands.BookIdentificationCommandBase
{
	[TestFixture]
	public class ValidateTests
	{
		...
		{
			IBookIdentificationCommand sut = CreateSut();
		...
		private IBookIdentificationCommand CreateSut()
		{
			_claimResolverMock.Setup(m => m.IsMediaLibraryModifier())
				.Returns(_fixture.Create<bool>());

			return new MyBookIdentificationCommand(Guid.NewGuid());
		}

		private class MyBookIdentificationCommand : BusinessLogic.MediaLibrary.Commands.BookIdentificationCommandBase
		{
			#region Constructor

			public MyBookIdentificationCommand(Guid mediaIdentifier) 
