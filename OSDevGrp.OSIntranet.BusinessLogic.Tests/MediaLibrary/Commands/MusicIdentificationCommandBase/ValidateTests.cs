...
namespace OSDevGrp.OSIntranet.BusinessLogic.Tests.MediaLibrary.Commands.MusicIdentificationCommandBase
{
	[TestFixture]
	public class ValidateTests
	{
		...
		{
			IMusicIdentificationCommand sut = CreateSut();
		...
		private IMusicIdentificationCommand CreateSut()
		{
			_claimResolverMock.Setup(m => m.IsMediaLibraryModifier())
				.Returns(_fixture.Create<bool>());

			return new MyMusicIdentificationCommand(Guid.NewGuid());
		}

		private class MyMusicIdentificationCommand : BusinessLogic.MediaLibrary.Commands.MusicIdentificationCommandBase
		{
			#region Constructor

			public MyMusicIdentificationCommand(Guid mediaIdentifier) 
