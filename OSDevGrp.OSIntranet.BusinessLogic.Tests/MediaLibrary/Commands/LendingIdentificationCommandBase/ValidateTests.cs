...
namespace OSDevGrp.OSIntranet.BusinessLogic.Tests.MediaLibrary.Commands.LendingIdentificationCommandBase
{
	[TestFixture]
	public class ValidateTests
	{
		...
		{
			ILendingIdentificationCommand sut = CreateSut();
		...
		private ILendingIdentificationCommand CreateSut()
		{
			_claimResolverMock.Setup(m => m.IsMediaLibraryLender())
				.Returns(_fixture.Create<bool>());

			return new MyLendingIdentificationCommand(Guid.NewGuid());
		}

		private class MyLendingIdentificationCommand : BusinessLogic.MediaLibrary.Commands.LendingIdentificationCommandBase
		{
			#region Constructor

			public MyLendingIdentificationCommand(Guid lendingIdentifier) 
