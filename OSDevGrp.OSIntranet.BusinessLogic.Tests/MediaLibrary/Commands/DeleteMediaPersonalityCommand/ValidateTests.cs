...
namespace OSDevGrp.OSIntranet.BusinessLogic.Tests.MediaLibrary.Commands.DeleteMediaPersonalityCommand
{
	[TestFixture]
	public class ValidateTests
	{
		...
		{
			IDeleteMediaPersonalityCommand sut = CreateSut();
		...
		public void Validate_WhenCalled_AssertShouldBeKnownValueWasCalledOnObjectValidatorWithMediaPersonalityIdentifier()
		{
			...
			IDeleteMediaPersonalityCommand sut = CreateSut(mediaPersonalityIdentifier);
		...
		[Test]
		[Category("UnitTest")]
		public void Validate_WhenCalled_AssertShouldBeDeletableWasCalledOnObjectValidatorWithMediaPersonalityIdentifier()
		{
			Guid mediaPersonalityIdentifier = Guid.NewGuid();
			IDeleteMediaPersonalityCommand sut = CreateSut(mediaPersonalityIdentifier);
			...
			_validatorMockContext.ObjectValidatorMock.Verify(m => m.ShouldBeDeletable(
					It.Is<Guid>(value => value == mediaPersonalityIdentifier),
					It.IsNotNull<Func<Guid, Task<IMediaPersonality>>>(),
					It.Is<Type>(value => value != null && value == sut.GetType()),
					It.Is<string>(value => string.IsNullOrWhiteSpace(value) == false && string.CompareOrdinal(value, "MediaPersonalityIdentifier") == 0),
					It.Is<bool>(value => value == false)),
				Times.Once);
		...
		private IDeleteMediaPersonalityCommand CreateSut(Guid? mediaPersonalityIdentifier = null)
		{
			_claimResolverMock.Setup(m => m.IsMediaLibraryModifier())
				.Returns(_fixture.Create<bool>());

			return MediaLibraryCommandFactory.BuildDeleteMediaPersonalityCommand(mediaPersonalityIdentifier ?? Guid.NewGuid());
		}
	}
}