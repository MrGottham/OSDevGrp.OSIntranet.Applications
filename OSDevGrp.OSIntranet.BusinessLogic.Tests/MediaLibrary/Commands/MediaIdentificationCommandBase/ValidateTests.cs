...
namespace OSDevGrp.OSIntranet.BusinessLogic.Tests.MediaLibrary.Commands.MediaIdentificationCommandBase
{
	...
	public class ValidateTests
	{
		...
		{
			IMediaIdentificationCommand sut = CreateSut();
		...
		private IMediaIdentificationCommand CreateSut()
		{
			_claimResolverMock.Setup(m => m.IsMediaLibraryModifier())
				.Returns(_fixture.Create<bool>());

			return new MyMediaIdentificationCommand(Guid.NewGuid());
		}

		private class MyMediaIdentificationCommand : BusinessLogic.MediaLibrary.Commands.MediaIdentificationCommandBase
		{
			#region Constructor

			public MyMediaIdentificationCommand(Guid mediaIdentifier) 
