...
namespace OSDevGrp.OSIntranet.BusinessLogic.Tests.MediaLibrary.Commands.CreateBorrowerCommand
{
	[TestFixture]
	public class ValidateTests
	{
		...
		{
			ICreateBorrowerCommand sut = CreateSut();
		...
		[Test]
		[Category("UnitTest")]
		public void Validate_WhenCalled_AssertShouldBeKnownValueWasNotCalledOnObjectValidatorWithBorrowerIdentifier()
		{
			Guid borrowerIdentifier = Guid.NewGuid();
			ICreateBorrowerCommand sut = CreateSut(borrowerIdentifier);

			sut.Validate(_validatorMockContext.ValidatorMock.Object, _claimResolverMock.Object, _mediaLibraryRepositoryMock.Object, _commonRepositoryMock.Object);

			_validatorMockContext.ObjectValidatorMock.Verify(m => m.ShouldBeKnownValue(
					It.Is<Guid>(value => value == borrowerIdentifier),
		...
		[Test]
		[Category("UnitTest")]
		public void Validate_WhenCalled_AssertShouldBeUnknownValueWasCalledOnObjectValidatorWithBorrowerIdentifier()
		{
			Guid borrowerIdentifier = Guid.NewGuid();
			ICreateBorrowerCommand sut = CreateSut(borrowerIdentifier);
			...
			_validatorMockContext.ObjectValidatorMock.Verify(m => m.ShouldBeUnknownValue(
					...
					It.Is<string>(value => string.IsNullOrWhiteSpace(value) == false && string.CompareOrdinal(value, "BorrowerIdentifier") == 0),
		...
		private ICreateBorrowerCommand CreateSut(Guid? borrowerIdentifier = null)
		{
			_claimResolverMock.Setup(m => m.IsMediaLibraryLender())
				.Returns(_fixture.Create<bool>());

			return MediaLibraryCommandFactory.BuildCreateBorrowerCommand(borrowerIdentifier ?? Guid.NewGuid(), _fixture.Create<string>(), _random.Next(100) > 50 ? _fixture.Create<string>() : null, _random.Next(100) > 50 ? _fixture.Create<string>() : null, _random.Next(100) > 50 ? _fixture.Create<string>() : null, _fixture.Create<int>());
		}
	}
}