...
namespace OSDevGrp.OSIntranet.BusinessLogic.Tests.MediaLibrary.Commands.DeleteLendingCommand
{
	[TestFixture]
	public class ValidateTests
	{
		...
		{
			IDeleteLendingCommand sut = CreateSut();
		...
		[Test]
		[Category("UnitTest")]
		public void Validate_WhenCalled_AssertShouldBeKnownValueWasCalledOnObjectValidatorWithLendingIdentifier()
		{
			Guid lendingIdentifier = Guid.NewGuid();
			IDeleteLendingCommand sut = CreateSut(lendingIdentifier);
		...
		[Test]
		[Category("UnitTest")]
		public void Validate_WhenCalled_AssertShouldBeUnknownValueWasNotCalledOnObjectValidatorWithLendingIdentifier()
		{
			Guid lendingIdentifier = Guid.NewGuid();
			IDeleteLendingCommand sut = CreateSut(lendingIdentifier);
		...
		[Test]
		[Category("UnitTest")]
		public void Validate_WhenCalled_AssertShouldBeDeletableWasCalledOnObjectValidatorWithLendingIdentifier()
		{
			Guid lendingIdentifier = Guid.NewGuid();
			IDeleteLendingCommand sut = CreateSut(lendingIdentifier);
			...
			_validatorMockContext.ObjectValidatorMock.Verify(m => m.ShouldBeDeletable(
					It.Is<Guid>(value => value == lendingIdentifier),
					It.IsNotNull<Func<Guid, Task<ILending>>>(),
					It.Is<Type>(value => value != null && value == sut.GetType()),
					It.Is<string>(value => string.IsNullOrWhiteSpace(value) == false && string.CompareOrdinal(value, "LendingIdentifier") == 0),
					It.Is<bool>(value => value == false)),
				Times.Once);
		...
		private IDeleteLendingCommand CreateSut(Guid? lendingIdentifier = null)
		{
			_claimResolverMock.Setup(m => m.IsMediaLibraryModifier())
				.Returns(_fixture.Create<bool>());

			return MediaLibraryCommandFactory.BuildDeleteLendingCommand(lendingIdentifier ?? Guid.NewGuid());
		}
	}
}