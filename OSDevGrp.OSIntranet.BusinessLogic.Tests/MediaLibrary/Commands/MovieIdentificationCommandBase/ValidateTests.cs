...
namespace OSDevGrp.OSIntranet.BusinessLogic.Tests.MediaLibrary.Commands.MovieIdentificationCommandBase
{
	[TestFixture]
	public class ValidateTests
	{
		...
		{
			IMovieIdentificationCommand sut = CreateSut();
		...
		private IMovieIdentificationCommand CreateSut()
		{
			_claimResolverMock.Setup(m => m.IsMediaLibraryModifier())
				.Returns(_fixture.Create<bool>());

			return new MyMovieIdentificationCommand(Guid.NewGuid());
		}

		private class MyMovieIdentificationCommand : BusinessLogic.MediaLibrary.Commands.MovieIdentificationCommandBase
		{
			#region Constructor

			public MyMovieIdentificationCommand(Guid mediaIdentifier) 
