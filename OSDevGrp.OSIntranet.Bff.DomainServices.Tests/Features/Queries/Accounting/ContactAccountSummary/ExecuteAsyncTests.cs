...
public class ExecuteAsyncTests : AccountIdentificationFeatureTestBase
{
    ...
    public async Task ExecuteAsync_WhenCalled_ReturnsContactAccountSummaryResponseWhereStaticTextsContainsExpectedStaticTextKey(StaticTextKey staticTextKey)
    {
        ...
        Mock<IContactAccountTextsBuilder>? contactAccountTextsBuilderMock = new Mock<IContactAccountTextsBuilder>();
        ...
        contactAccountTextsBuilderMock!.Setup(m => m.BuildAsync(It.IsAny<ContactAccountModel>(), It.IsAny<IFormatProvider>(), It.IsAny<CancellationToken>()))
            .Returns(Task.FromResult(new Mock<IContactAccountTexts>().Object));
        emptyRuleSetBuilderMock!.Setup();
        accountingGatewayMock!.Setup(m => m.GetContactAccountAsync(It.IsAny<int>(), It.IsAny<string>(), It.IsAny<DateTimeOffset>(), It.IsAny<CancellationToken>()))
            .Returns(Task.FromResult(fixture!.CreateContactAccountModel(random!)));

        IQueryFeature<ContactAccountSummaryRequest, ContactAccountSummaryResponse> sut = new OSDevGrp.OSIntranet.Bff.DomainServices.Features.Queries.Accounting.ContactAccountSummary.ContactAccountSummaryFeature(permissionCheckerMock!.Object, accountingGatewayMock!.Object, staticTextProviderMock!.Object, contactAccountTextsBuilderMock!.Object, emptyRuleSetBuilderMock!.Object);

        ContactAccountSummaryRequest contactAccountSummaryRequest = CreateContactAccountSummaryRequest(fixture!);
        ContactAccountSummaryResponse result = await sut.ExecuteAsync(contactAccountSummaryRequest);

        Assert.That(result.StaticTexts.ContainsKey(staticTextKey), Is.True);
    }

    private static ContactAccountSummaryRequest CreateContactAccountSummaryRequest(Fixture fixture, int? accountingNumber = null, string? accountNumber = null, DateTimeOffset? statusDate = null, IFormatProvider? formatProvider = null, ISecurityContext? securityContext = null)
    {
        return new ContactAccountSummaryRequest(Guid.NewGuid(), accountingNumber ?? fixture.Create<int>(), accountNumber ?? fixture.Create<string>(), statusDate ?? fixture.Create<DateTimeOffset>(), formatProvider ?? System.Globalization.CultureInfo.InvariantCulture, securityContext ?? CreateSecurityContext(fixture));
    }
}