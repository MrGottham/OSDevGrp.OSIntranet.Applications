...
namespace OSDevGrp.OSIntranet.Bff.DomainServices.Tests.Features.Queries.Accounting.BudgetAccountSummary;
...
public class ExecuteAsyncTests : AccountIdentificationFeatureTestBase
{
    ...
    public async Task ExecuteAsync_WhenCalled_ReturnsBudgetAccountSummaryResponseWhereStaticTextsContainsExpectedStaticTextKey(StaticTextKey staticTextKey)
    {
        ...
        Mock<IBudgetAccountTextsBuilder>? budgetAccountTextsBuilderMock = new Mock<IBudgetAccountTextsBuilder>();
        ...
        budgetAccountTextsBuilderMock!.Setup(m => m.BuildAsync(It.IsAny<BudgetAccountModel>(), It.IsAny<IFormatProvider>(), It.IsAny<CancellationToken>()))
            .Returns(Task.FromResult(new Mock<IBudgetAccountTexts>().Object));
        emptyRuleSetBuilderMock!.Setup();
        accountingGatewayMock!.Setup(m => m.GetBudgetAccountAsync(It.IsAny<int>(), It.IsAny<string>(), It.IsAny<DateTimeOffset>(), It.IsAny<CancellationToken>()))
            .Returns(Task.FromResult(fixture!.CreateBudgetAccountModel(random!)));

        IQueryFeature<BudgetAccountSummaryRequest, BudgetAccountSummaryResponse> sut = new OSDevGrp.OSIntranet.Bff.DomainServices.Features.Queries.Accounting.BudgetAccountSummary.BudgetAccountSummaryFeature(permissionCheckerMock!.Object, accountingGatewayMock!.Object, staticTextProviderMock!.Object, budgetAccountTextsBuilderMock!.Object, emptyRuleSetBuilderMock!.Object);

        BudgetAccountSummaryRequest budgetAccountSummaryRequest = CreateBudgetAccountSummaryRequest(fixture!);
        BudgetAccountSummaryResponse result = await sut.ExecuteAsync(budgetAccountSummaryRequest);

        Assert.That(result.StaticTexts.ContainsKey(staticTextKey), Is.True);
    }

    private static BudgetAccountSummaryRequest CreateBudgetAccountSummaryRequest(Fixture fixture, int? accountingNumber = null, string? accountNumber = null, DateTimeOffset? statusDate = null, IFormatProvider? formatProvider = null, ISecurityContext? securityContext = null)
    {
        return new BudgetAccountSummaryRequest(Guid.NewGuid(), accountingNumber ?? fixture.Create<int>(), accountNumber ?? fixture.Create<string>(), statusDate ?? fixture.Create<DateTimeOffset>(), formatProvider ?? System.Globalization.CultureInfo.InvariantCulture, securityContext ?? CreateSecurityContext(fixture));
    }
}