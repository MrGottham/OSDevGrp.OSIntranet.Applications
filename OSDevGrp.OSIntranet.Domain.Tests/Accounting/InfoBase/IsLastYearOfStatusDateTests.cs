...
{
    [TestFixture]
    public class IsLastYearOfStatusDateTests
    {
        ...
        [TestCase(11)]
        [TestCase(12)]
        public async Task IsLastYearOfStatusDate_WhenStatusDateIsWithinNextYearOfInfo_ReturnsTrue(short month)
        {
            short year = (short) _random.Next(Sut.MinYear, Sut.MaxYear - 1);
            IInfo<IInfo> sut = CreateSut(year, month);

            DateTime statusDate = new DateTime(year + 1, month, _random.Next(1, DateTime.DaysInMonth(year + 1, month)));
            IInfo result = await sut.CalculateAsync(statusDate);

            Assert.That(result.IsLastYearOfStatusDate, Is.True);
        }
        ...
        [TestCase(11)]
        [TestCase(12)]
        public async Task IsLastYearOfStatusDate_WhenStatusDateIsDifferentMonthThanNextYearOfInfo_ReturnsTrue(short month)
        {
            short year = (short) _random.Next(Sut.MinYear, Sut.MaxYear - 1);
            IInfo<IInfo> sut = CreateSut(year, month);

            short differentMonth = GetNonMatchingMonth(month);
            DateTime statusDate = new DateTime(year + 1, differentMonth, _random.Next(1, DateTime.DaysInMonth(year + 1, differentMonth)));
            IInfo result = await sut.CalculateAsync(statusDate);
        ...
        [TestCase(11)]
        [TestCase(12)]
        public async Task IsLastYearOfStatusDate_WhenStatusDateIsDifferentYearThanInfo_ReturnsFalse(short month)
        {
            short year = (short) _random.Next(Sut.MinYear, Sut.MaxYear - 1);
            IInfo<IInfo> sut = CreateSut(year, month);

            short differentYear = GetNonMatchingYear((short) (year + 1));
            DateTime statusDate = new DateTime(differentYear, month, _random.Next(1, DateTime.DaysInMonth(differentYear, month)));
            IInfo result = await sut.CalculateAsync(statusDate);

            Assert.That(result.IsLastYearOfStatusDate, Is.False);
        }
