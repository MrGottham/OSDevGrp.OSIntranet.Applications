...
public class GetAccountGroupsAsyncTests : ServiceGatewayTestBase
{
    ...
    public async Task GetAccountGroupsAsync_WhenCalled_AssertAccountgroupsAsyncWasCalledOnWebApiClient()
    {
        ...
        _webApiClientMock!.Verify(m => m.AccountgroupsAsync(It.Is<string>(value => value == ApiVersions.Version1_0), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task GetAccountGroupsAsync_WhenCalled_AssertAccountgroupsAsyncWasCalledOnWebApiClientWithCancellationTokenEqualToCancellationTokenFromArguments()
    {
        ...
        _webApiClientMock!.Verify(m => m.AccountgroupsAsync(It.Is<string>(value => value == ApiVersions.Version1_0), It.Is<CancellationToken>(value => value == cancellationToken)), Times.Once);
    }
    ...
    private IAccountingGateway CreateSut(ICollection<AccountGroupModel>? accountGroupModels = null, Exception? exception = null)
    {
        if (exception != null)
        {
            _webApiClientMock!.Setup(m => m.AccountgroupsAsync(It.Is<string>(value => value == ApiVersions.Version1_0), It.IsAny<CancellationToken>()))
                .Throws(exception);
        }
        else
        {
