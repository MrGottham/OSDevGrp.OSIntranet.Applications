...
public class GetAccountAsyncTests : ServiceGatewayTestBase
{
    ...
    private IAccountingGateway CreateSut(AccountModel? accountModel = null, Exception? exception = null)
    {
        if (exception != null)
        {
            _webApiClientMock!.Setup(m => m.AccountsAsync(It.IsAny<int>(), It.IsAny<string>(), It.IsAny<DateTimeOffset?>(), It.Is<string>(value => value == ApiVersions.Version1_0), It.IsAny<CancellationToken>()))
                .Throws(exception);
        }
        else
        {
